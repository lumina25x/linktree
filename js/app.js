 ...
 * 상품명, 가격, 썸네일, 링크 중심의 초경량 렌더링 및 번호 검색 엔진
 */
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 1. 유입 채널 & 방문자 통계 실시간 집계 (UTM 파라미터 + Referrer)
  ...
  function initProfile() {
    channelAvatar.src = CONFIG.channel.avatar;
    channelTitle.textContent = CONFIG.channel.name;
    channelTagline.textContent = CONFIG.channel.tagline;
    topNoticeText.textContent = CONFIG.ui.topNoticeText;
    searchInput.placeholder = CONFIG.ui.searchPlaceholder;

    if (CONFIG.channel.businessEmail) {
      btnBusiness.href = `mailto:${CONFIG.channel.businessEmail}?subject=[비즈니스 제안] ${CONFIG.channel.name} 제휴 문의`;
  ...
  function renderSocials() {
    const socials = CONFIG.channel.socials;
    const icons = {
  ...
  async function handleShare() {
    const shareData = {
      title: `${CONFIG.channel.name} | 영상 속 꿀템 좌표`,
      text: `${CONFIG.channel.name} 프로필 링크입니다. 영상 속 번호를 검색해 보세요!`,
